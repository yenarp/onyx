...
_start:
    ...
    ; ensure we are executing at 0x0:0x7c00 as some bioses load CS to 0x7c0 and IP to 0.
    mov sp, zj_bytes
    retf
...
too_large_error:
    mov si, str_stage2_too_large
    call puts
    jmp panic

fs_corrupt:
    mov si, str_fat_corruption
    call puts

panic:
    mov si, str_key_reboot
    call puts
...
str_floppy_disk_error:
    db "Floppy disk error", endl, 0
str_stage2_not_found:
    db "STAGE2.BIN not found", endl, 0
str_stage2_too_large:
    db "STAGE2.BIN is too large", endl, 0
str_fat_corruption:
    db "FAT corruption", endl, 0
str_key_reboot:
    db "Press any key to reboot", endl, 0
