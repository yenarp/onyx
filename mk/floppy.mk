...
DOS_BOOT := $(BUILD_DIR)/MSDOS.HEX

FBOOT_BIN := $(BUILD_DIR)/boot/sector/floppy.bin
STAGE2 := $(if $(strip $(STAGE2_C_SRCS)),$(STAGE2_BIN),)
...
define floppy_copy_payloads
	if [ -f "$(STAGE2_BIN)" ]; then
		mcopy -i "$(IMG)" "$(STAGE2_BIN)" ::STAGE2.BIN
	fi
endef
...
define make_dos_floppy
	...
	dd if="$(DOS_IMG)" of="$(DOS_BOOT)" bs=512 count=1 status=none
	dd if="$(FBOOT_BIN)" of="$(IMG)" bs=1 count=512 conv=notrunc status=none
	...
	mcopy -i "$(IMG)" "$(DOS_BOOT)" ::MSDOS.HEX
endef

floppy: $(FBOOT_BIN) $(SPARK_HEX_OPT) $(STAGE2) $(LOOM_BIN) $(BOOT_INI) $(if $(strip $(DOS_DISK)),$(DOS_IMG),)
	$(call mkdir_p,$(BUILD_DIR))
